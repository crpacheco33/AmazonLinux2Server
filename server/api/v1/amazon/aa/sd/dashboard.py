...
def index(
    ...
    if campaign_ids:
        ...
        campaign_ids = [int(campaign.get('campaignId')) for campaign in sd_campaigns]
    else:
        campaign_ids = []
    ...
    response = source.sa_dashboard(
        ...
        to_date,
        objectives,
        ad_type=Constants.SPONSORED_DISPLAY,
        interval=interval,
    )

    log.info(
        f'Queried SD dashboard data',
    )
