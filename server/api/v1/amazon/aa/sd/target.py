...
async def update(
    request: Request,
    data: List[UpdateSDTargetSchema],
    brand: Any = Depends(
        brand,
    ),
    interface: Interface = Depends(
...
async def destroy(
    ...
    log.info(
        f'Archiving SD target {target_id}...',
    )
    ...
    log.info(
        f'Archived SD target {target_id}',
    )
